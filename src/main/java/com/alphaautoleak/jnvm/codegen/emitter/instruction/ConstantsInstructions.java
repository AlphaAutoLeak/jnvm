...
public class ConstantsInstructions {
    ...
    public static class LdcInstruction extends Instruction {
        ...
        private void generateLdcBody(PrintWriter w, String indent) {
            ...
            w.println(indent + "    }");
            w.println(indent + "    case META_METHOD: {");
            w.println(indent + "        if (meta->handleTag <= 0) { break; }");
            w.println(indent + "        vm_indy_init_cache(env);");
            w.println(indent + "        jobject classLoader = NULL;");
            w.println(indent + "        if (frame.callerClass && id_getClassLoaderMid) {");
            ...
            w.println(indent + "            break;");
            w.println(indent + "        }");
            w.println(indent + "        frame.stack[frame.sp++].l = mh; break;");
            w.println(indent + "    }");
